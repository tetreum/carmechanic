...
namespace FMODUnity
{
 ...
 Instance.studioSystem.loadBankMemory(www.bytes, FMOD.Studio.LOAD_BANK_FLAGS.NORMAL, out loadedBank.Bank);
...
#if UNITY_ADDRESSABLES_EXIST
        public static void LoadBank(AssetReference assetReference, bool loadSamples = false,
        ...
        private static void Asset_Completed(AsyncOperationHandle<TextAsset> obj)
        {
            Asset_Completed(obj, false);
        }

        private static void Asset_Completed(AsyncOperationHandle<TextAsset> obj, bool loadSamples)
        {
            if (!obj.IsValid())
            {
