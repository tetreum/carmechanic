...
namespace FMOD.Studio
{
    ...
    public struct USER_PROPERTY
    {
        ...
        private readonly Union_IntBoolFloatString value;
    ...
    public struct System
    {
        ...
        public RESULT loadBankMemory(byte[] buffer, LOAD_BANK_FLAGS flags, out Bank bank)
        {
            ...
            var result = FMOD_Studio_System_LoadBankMemory(handle, pointer, buffer.Length, LOAD_MEMORY_MODE.LOAD_MEMORY,
                flags, out bank.handle);
            pinnedArray.Free();
            return result;
        }
    ...
    public struct EventDescription
    {
        ...
        public RESULT getUserProperty(string name, out USER_PROPERTY property)
        {
            using (var encoder = StringHelper.GetFreeHelper())
            {
                return FMOD_Studio_EventDescription_GetUserProperty(handle, encoder.byteFromStringUTF8(name),
                    out property);
            }
        }
        ...
        [DllImport(STUDIO_VERSION.dll)]
        private static extern RESULT FMOD_Studio_EventDescription_GetMinimumDistance(IntPtr eventdescription,
            out float distance);

        [DllImport(STUDIO_VERSION.dll)]
        private static extern RESULT FMOD_Studio_EventDescription_GetMaximumDistance(IntPtr eventdescription,
            out float distance);
